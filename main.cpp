...
int main() {
    ...
    while(1) {
        ...
        ThisThread::sleep_for(200ms); // Stop the program counter here and wait for 200 ms. In the meanwhile the led is on
        myled = 0; // Switch-off the LED
        ThisThread::sleep_for(1s); // Stop the program counter here and wait for 1 ms. In the meanwhile the led is off
    }
}
